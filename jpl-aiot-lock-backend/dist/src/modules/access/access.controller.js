...
async function listByLock(req, res, next) {
    try {
...
async function revoke(req, res, next) {
    try {
        return res.json({
            ok: true,
            data: await accessService.revokeAccess(req.params.lockId, req.params.accessId, req.user?.id),
        });
    }
    catch (error) {
        return next(error);
